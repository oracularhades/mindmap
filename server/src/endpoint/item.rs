...
use crate::internal::item::item_get;
use crate::responses::*;
use crate::structs::*;
use crate::tables::*;
...
pub async fn item_list(mut db: Connection<Db>, folder: Option<String>, params: &Query_string) -> Custom<Value> {
    ...
    let (item_result, error_to_respond_with, item_db) = crate::internal::item::item_list(db, request_authentication_output.user_id.clone(), folder.clone()).await.expect("Failed to list items");
    db = item_db;
